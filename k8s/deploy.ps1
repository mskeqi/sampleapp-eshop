...
if ($deployInfrastructure) {
    ...
    # Mooncake special, update image to load from proxy
    Write-Host "Update infrastructure Image containers to use $registry" -ForegroundColor Yellow
    ExecKube -cmd 'set image deployments/sql-data sql-data=${registry}/microsoft/mssql-server-linux'
    ExecKube -cmd 'set image deployments/basket-data basket-data=${registry}/library/redis:3.2-alpine'
